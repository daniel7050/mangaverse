...
export default function Browse() {
  ...
  const fetchManga = useCallback(async (pageNum = 1, append = false) => {
    ...
    try {
      ...
      if (sort === 'popular') params.sort = 'viewCount';
      if (sort === 'rating') params.sort = 'rating';
      if (sort === 'az') params.sort = 'title';
        ...
        <aside className="browse-sidebar">
          ...
          <div className="filter-group">
            ...
            {STATUS_OPTIONS.map(s => (
              <button
                key={s}
                className={`filter-pill ${(status || 'all') === s ? 'active' : ''}`}
            ...
            <div className="browse-active-filters">
              {genre && (
                <span className="active-tag">
                  {genre}
                  <button onClick={() => setFilter('genre', '')}>×</button>
                </span>
              )}
              {status !== 'all' && (
                <span className="active-tag">
                  {status}
                  <button onClick={() => setFilter('status', 'all')}>×</button>
                </span>
              )}
            </div>
          )}
